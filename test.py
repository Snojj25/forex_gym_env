...
from envs.forex_env.A3C.model import ActorCritic
...
# h_t  = torch.zeros((60, 64), requires_grad = True)
# c_t  = torch.zeros((60, 64), requires_grad = True)
...
# value, action_values, hidden_list = model((state, hidden_list))
# print(0 % 500 == 0)
...
#             if count == 0:
#                 hx  = torch.rand((3, 60, 64), requires_grad = True)
#                 cx  = torch.rand((3, 60, 64), requires_grad = True)
            
#             inputs = x, (hx,cx)
#             # Clear gradients
#             optimizer.zero_grad()
#             # Forward pass
#             out, (hx,cx) = model(inputs)
#             # Calculate loss
#             loss = loss_fn(out, y)
#             # Calculate gradients
#             loss.backward()
